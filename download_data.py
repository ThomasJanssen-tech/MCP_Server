...
if not file_exists:
    ...
    for ind in companies.index:

        json_data.append({"url":companies.loc[ind, "url"]})
...
#################################################################################################################################################################
###############################   3.  IF SnapshotID IS SET, FETCH THE SCRAPED DATA AND STORE IN data DIR   ######################################################
#################################################################################################################################################################


###############################   CHECK WHETHER ALL WEBSITES ARE SCRAPED   #######################################################################################
...
###############################   IF ALL WEBSITES ARE READY, FETCH DATA AND WRITE TO FILES   ######################################################################
    ...
    if snapshot_ready:
        print("== > Scraping is done - fetching data...")
        ...
        with open(os.getenv("DATASET_STORAGE_FOLDER")+"data.txt", "wb") as f:
         ...
         print("== > Data is not ready yet. Please try again later.")